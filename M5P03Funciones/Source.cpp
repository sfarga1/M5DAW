...
bool ataqueEnemigo1() {
	if (enemy1IsAlive)
	{
		...
		cout << "El " << enemy1Name << " te ha atacado!" << endl;
...
bool ataqueEnemigo2() {
	if (enemy2IsAlive)
	{
		...
		cout << "El " << enemy2Name << " te ha atacado!" << endl;
		cout << "Te quedan " << heroHP << " puntos de vida" << endl << endl;
...
bool ataqueEnemigo3() {
	if (enemy3IsAlive)
	{
		...
		cout << "El " << enemy3Name << " te ha atacado!" << endl;
		cout << "Te quedan " << heroHP << " puntos de vida" << endl << endl;
...
bool ataqueEnemigo4() {
	if (enemy4IsAlive)
	{
		...
		cout << "El " << enemy4Name << " te ha atacado!" << endl;
		cout << "Te quedan " << heroHP << " puntos de vida" << endl << endl;
...
int heroChoosesEnemyToAttackPath1() {
	int selectorEnemigo;
...
int heroChoosesEnemyToAttackPath2() {
	int selectorEnemigo;
	cout << heroName << ", a quien quieres atacar?" << endl;
	cout << "[1] " << enemy3Name << endl << "[2] " << enemy4Name << endl;
	cin >> selectorEnemigo;
	if (selectorEnemigo == 1)
	{
		return 1;
...
int main() {
	...
	case 1: //path 1
		while ((heroIsAlive) && ((enemy1IsAlive) || (enemy2IsAlive))) { //hacer varios bucles, segun que enemigo este vivo.
			...
			//Enemigo 1 ataca
			heroIsAlive = ataqueEnemigo1();
			//Enemigo 2 ataca
			heroIsAlive = ataqueEnemigo2();
			...
			if (heroHP > 0)
			{
				...
				atacarUnEnemigo = heroChoosesEnemyToAttackPath1();
		...
		else
		{
			...
			else if (opcion2 == 2)
			{
				while ((heroIsAlive) && ((enemy3IsAlive) || (enemy4IsAlive))) { //De aqui en adelante es todo exactamente igual que en el bucle de batalla del path 1 (exceptuando los mensajes de victoria y derrota), cambiando al enemy1 por enemy3 y al enemy2 por enemy4.
					...
					//Enemigo 3 ataca
					heroIsAlive = ataqueEnemigo3();
					//Enemigo 4 ataca
					heroIsAlive = ataqueEnemigo4();
					//Heroe ataca
					if (heroHP > 0)
					{
						...
						//Heroe elige a que enemigo atacar
						atacarUnEnemigo = heroChoosesEnemyToAttackPath2();
